...
from data_io.instructions import get_all_instructions, load_instruction_data_from_json
from data_io.instructions import get_word_to_token_map, merge_instruction_sets
from data_io.paths import get_instruction_cache_dir, get_results_path, get_results_dir
...
class PolicyRoller:
    ...
    """
    def __init__(self, instance_id=0, run_name='', evaler=None):
        ...
        self.stage_param = get_current_parameters()["ModelPVN"]
    ...
    def load_all_envs(self):
        ...
        if setup["eval_env_set"] in ["test", "dev", "train"]:
            ...
            all_instructions = load_instruction_data_from_json(other_file)
    ...
    def tokenize_string(self, s: str):
        # for testing the llm-rephrased instruction on original model (pvn: previous work)
        s = s.replace('(', ' lrb ')\
            .replace('(', ' lrb ')\
                .replace(')', ' rrb ')\
                .replace('-', ' ')\
        ...
        for word in new_word_list:
            if word in self.word2token:
                token_instruction.append(self.word2token[word])
    ...
    def roll_out_on_env(self, params, instructions_set, set_idx, only_seg_idx=None, custom_instr=None):
        ...
        for segment, seg_idx in valid_segments:
            ...
            # dagger instruction tokenization or not in sbert / sentence-transformers
            from data_io.instructions import tokenize_instruction
            if not self.stage_param["use_sbert"]:
                token_instruction = self.tokenize_string(string_instruction)
            else:
                token_instruction = [tokenize_instruction(string_instruction, None, use_token=False)]
            ...
            while True:
                ...
                # if self.stage_param["predict_with_current_position"] and not self.stage_param["eval"]:
                #     current_position = state.state[:2]
                #     params.policy.update_gtr_traj_with_current_position(position=current_position)
            ...
            # Plot the trajectories for error tracking, Plot entire envs not segment by segment
            if params.save_plots:
                # for generate the pictures of map with trajectory
                self.presenter.plot_paths(segment_dataset, interactive=True)
    ...
    def roll_out_policy(self, params):
        ...
        try:
            ...
            if self.run_name != "train_dagger":
                path = get_results_path(self.run_name, makedir=True)
                if os.path.isfile(path):
                    with open(path, "r") as result_file:
                        file_content = result_file.read()
                        if file_content != '':
                            exist_roll_out = list(map(int, json.loads(file_content)["all_dist"].keys()))
                            valid_env_ids = list(set(valid_env_ids).difference(exist_roll_out))
                            valid_env_ids.sort()
            ...
            for i, env_id in enumerate(valid_env_ids):
                ...
                if len(instruction_sets[0]['instructions']) == 0:
                ...
                try:
                    for j, instructions_set in enumerate(instruction_sets):
                        ...
                        #log("Path finished!")
                        DebugWriter().commit()
                    if "dagger" not in self.run_name:
                        self.evaler.evaluate_dataset(dataset)
                        final_result = self.evaler.save_results()
                        final_result.pop("all_dist")
                        print("result after roll out on env {", env_id, "}: ", final_result)
                        dataset = []
