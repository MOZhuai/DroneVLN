...
def load_model(model_file_override=None, real=False, model_name_override=False):
    ...
    # -----------------------------------------------------------------------------------------------------------------
        ...
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_map=True, aux_grounding_map=True, aux_goal_map=True, aux_lang=False)
        pytorch_model = True
    elif model_name == "gsmn_wo_jgnd":
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_map=True, aux_grounding_map=False, aux_goal_map=True, aux_lang=True)
        pytorch_model = True
    elif model_name == "gsmn_wo_jclass":
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_map=False, aux_grounding_map=True, aux_goal_map=True, aux_lang=True)
        pytorch_model = True
    elif model_name == "gsmn_wo_jgoal":
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_map=True, aux_grounding_map=True, aux_goal_map=False, aux_lang=True)
        pytorch_model = True
        ...
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_features=False, aux_grounding_features=False,
                         aux_class_map=True, aux_grounding_map=True, aux_goal_map=True, aux_lang=True,
                         pos_noise=True, rot_noise=False)
        pytorch_model = True
    elif model_name == "gsmn_w_rotnoise":
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_features=False, aux_grounding_features=False,
                         aux_class_map=True, aux_grounding_map=True, aux_goal_map=True, aux_lang=True,
                         pos_noise=False, rot_noise=True)
        pytorch_model = True
    elif model_name == "gsmn_w_bothnoise":
        model = ModelRSS(run_name, model_class=ModelRSS.MODEL_RSS,
                         aux_class_features=False, aux_grounding_features=False,
                         aux_class_map=True, aux_grounding_map=True, aux_goal_map=True, aux_lang=True,
                         pos_noise=True, rot_noise=True)
        pytorch_model = True

    elif model_name == "gs_fpv":
        model = ModelGSFPV(run_name, aux_class_features=True, aux_grounding_features=True, aux_lang=True,
                           recurrence=False)
        pytorch_model = True
    ...
    # -----------------------------------------------------------------------------------------------------------------
        ...
        model = ModelTrajectoryProbRatio(run_name, model_class=mtpr.PVN_STAGE1_ONLY)
        pytorch_model = True
    ...
    if pytorch_model:
        ...
        if model_file:
            ...
            logger_info("Loaded previous model: " + model_file)
            model_loaded = True
        if cuda:
            model = model.cuda()
