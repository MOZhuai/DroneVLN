...
from env_config.definitions.landmarks import get_landmark_name_to_index, NUM_LANDMARKS
from env_config.definitions.nlp_templates import N_LANDMARKS, N_SIDES, get_side_name2idx
from data_io.instructions import split_instruction, clean_instruction, words_to_terms, load_thesaurus, get_instruction_segment
...
from transformations import cf_to_img, as_to_img
...
PROVIDER_TRAJECTORY_GROUND_TRUTH_STATIC_CURRENT = "trajectory_map_static_current"
...
def get_top_down_ground_truth_static_current_global(env_id, start_idx, end_idx, img_w, img_h, map_w, map_h, position):
    """
    Returns the ground-truth label oriented in the global map frame
    :param env_id:
    :param start_idx:
    ...
    path = path[start_idx:end_idx]
    # instruction_segments = [self.all_instr[env_id][set_idx]["instructions"][seg_idx]]
    seg_labels = np.zeros([img_w, img_h, 3]).astype(float)
    ...
    seg_labels[:, :, 0] = tdd.plot_path_on_img(seg_labels[:, :, 0], path_in_img)
    if len(path_in_img) > 1:
        seg_labels[:, :, 1] = tdd.plot_path_on_img(
            seg_labels[:, :, 1],
            path_in_img[find_nearest_point(position, path):-1]
        )
        seg_labels[:, :, 2] = tdd.plot_dot_on_img(seg_labels[:, :, 2], path_in_img[-1], gauss_sigma)

    seg_labels[:, :, 0] = gaussian_filter(seg_labels[:, :, 0], gauss_sigma)
    seg_labels[:, :, 1] = gaussian_filter(seg_labels[:, :, 1], gauss_sigma)
    seg_labels[:, :, 2] = gaussian_filter(seg_labels[:, :, 2], gauss_sigma)

    # Standardize both channels separately (each has mean zero, unit variance)
    seg_labels_path = standardize_2d_prob_dist(seg_labels[:, :, 0:1])
    seg_labels_cur_path = standardize_2d_prob_dist(seg_labels[:, :, 1:2])
    seg_labels_endpt = standardize_2d_prob_dist(seg_labels[:, :, 2:3])

    seg_labels = np.concatenate((seg_labels_path, seg_labels_cur_path, seg_labels_endpt), axis=0)
...
def provider_lm_pos_lm_indices_fpv(segment_data, data):
    ...
    params = P.get_current_parameters().get("ModelPVN") or P.get_current_parameters().get("Model")
...
def provider_trajectory_ground_truth(segment_data, data, kind="static"):
    ...
    model_params = P.get_current_parameters()["ModelPVN"]
    ...
    for timestep in range(traj_len):
        ...
        if segment_data[timestep] is not None and obs_mask[timestep]:
            ...
            elif kind == "static_current":
                position = data["states"][timestep][0:2]
                labels_t = get_top_down_ground_truth_static_current_global(
                    env_id, start_idx, end_idx, m_size, m_size, w_size, w_size, position
                )
            else:
...
def provider_trajectory_ground_truth_static_current(segment_data, data):
    return provider_trajectory_ground_truth(segment_data, data, "static_current")
...
def provider_past_trajectory(segment_data, data):
    ...
    for timestep in range(traj_len):
        ...
        pos_map = as_to_img(pos_as[np.newaxis, :], img_size=32)[0]
        if last_pos != None:
...
def resolve_data_provider(aux_provider_name):
    ...
    elif aux_provider_name == PROVIDER_TRAJECTORY_GROUND_TRUTH_STATIC_CURRENT:
        return provider_trajectory_ground_truth_static_current
...
def get_stackable_label_names(aux_provider_names):
    ...
    for provider in aux_provider_names:
        ...
        elif provider == PROVIDER_TRAJECTORY_GROUND_TRUTH_STATIC_CURRENT:
            label_names += ["traj_ground_truth"]
        elif provider == PROVIDER_TRAJECTORY_GROUND_TRUTH_DYNAMIC:
            label_names += ["traj_ground_truth"]
        elif provider == PROVIDER_TRAJECTORY_GROUND_TRUTH_DYNAMIC_NOISY:
