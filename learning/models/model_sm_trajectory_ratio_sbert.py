...
class ModelTrajectoryTopDown(ModuleWithAuxiliaries):
    ...
    def __init__(self, run_name="", model_class=MODEL_FPV):
        ...
        self.map_processor_b_r = RatioPathPredictor(
            ...
            posterior_channels=self.num_map_channels,
        ...
        if self.use_aux_grounding_on_map:
            enable_weight_saving(self.map_processor_a_w.lang_filter, "lang_filter_map_grounding",
                                 alwaysfreeze=self.act_only)
        ...
        if self.use_aux_class_features:
            self.add_auxiliary(
                ClassAuxiliary2D("aux_class", None, self.params["feature_channels"], self.params["num_landmarks"], 0,
        ...
        if self.use_aux_class_on_map:
            self.add_auxiliary(
                ClassAuxiliary2D("aux_class_map", self.params["world_size_px"], self.params["feature_channels"],
                                 self.params["num_landmarks"], 0,
                                 "map_s_w_select", "lm_pos_map_select", "lm_indices_select"))
        if self.use_aux_grounding_on_map:
            self.add_auxiliary(
                ClassAuxiliary2D("aux_grounding_map", self.params["world_size_px"], self.params["relevance_channels"],
                                 2, 0,
                                 "map_a_w_select", "lm_pos_map_select", "lm_mentioned_select"))
        if self.use_aux_goal_on_map:
            self.add_auxiliary(
                GoalAuxiliary2D("aux_goal_map", self.params["goal_channels"], self.params["world_size_px"],
        ...
        # CoRL model uses alignment-model groundings
            ...
            self.add_auxiliary(
                ClassAuxiliary("aux_lang_lm_nl", self.params["emb_size"], 2, self.params["num_landmarks"],
    ...
    def scale_images(self, images):
        if images.shape[0] == 72 and images.shape[1] == 144:
            return images
        images = np.array(Image.fromarray(images).resize((144, 72)))
        return images
    ...
    def get_action(self, state, instruction):
        ...
        with torch.no_grad():
            ...
            action = self(img_in_t, state, instruction, plan=plan_now, pos_enc=step_enc,
    ...
    def forward(self, images, states, instructions,
                has_obs=None, plan=None, save_maps_only=False, pos_enc=None, noisy_poses=None, start_poses=None,
                firstseg=None):
        """
        ...
        if (not self.params["train_action_only"] or not self.params["train_action_from_dstar"] or not self.params[
            "run_action_from_dstar"]) \
            ...
            # show_img(maps_w_select[:, 0:1].cpu().detach(), "drif/01acc_map", True)
            maps_w_select, map_poses_w_select, cam_poses_select, noisy_poses_select, start_poses_select, \
                sent_embeddings_select, pos_enc = self.batch_select(maps_w, map_poses_w, cam_poses, noisy_poses,
                                                                    start_poses, sent_embeddings, pos_enc, plan)
            ...
            if len(maps_w_select) > 0:
                ...
                maps_w_select, map_poses_w_select = self.map_processor_a_w(maps_w_select, sent_embeddings_select,
                                                                           map_poses_w_select, show="")
                ...
                # show_img(maps_s_select[:, 0:1].cpu().detach(), "egocentric_map")
    ...
    def sup_loss_on_batch(self, batch, eval):
        ...
        for b in range(batch_size):
            ...
            actions = self(b_images, b_states, b_instructions,
                           has_obs=b_obs_mask, plan=b_plan_mask, pos_enc=b_pos_enc, firstseg=b_firstseg_mask,
                           noisy_poses=b_noisy_poses, start_poses=b_start_poses)
    ...
    def get_dataset(self, data=None, envs=None, dataset_name=None, eval=False):
        ...
        return SegmentDataset(data=data, env_list=envs, dataset_name=dataset_name, aux_provider_names=data_sources,
                              segment_level=True, use_token=False)
