...
class ModelTrajectoryTopDown(ModuleWithAuxiliaries):
    ...
    def __init__(self, run_name="", model_class=MODEL_FPV):
        ...
        if self.params["condition_on_start_pos"]:
            self.map_processor_a_w2 = DrawStartPosOnGlobalMap(self.params["global_map_size"], self.params["world_size_px"])
        else:
            self.map_processor_a_w2 = IdentityMapProcessor(source_map_size=self.params["global_map_size"], world_size=self.params["world_size_px"])
    ...
    def save_viz(self, images_in):
        imsave(get_viz_dir(self.env_id, self.seg_idx) + "fpv_" + str(self.seq_step) + ".png", images_in)
        ...
        save_tensor_as_img(trajpred_posterior, "D_u" + str(self.seq_step), self.env_id, self.seg_idx, renorm_each_channel=True)
        ...
        save_tensor_as_img(trajpred_posterior_r, "D_u_r" + str(self.seq_step), self.env_id, self.seg_idx, renorm_each_channel=True)
        ...
        save_tensor_as_img(trajpred_prior, "D_prior" + str(self.seq_step), self.env_id, self.seg_idx, renorm_each_channel=True)
    ...
    def forward(self, images, states, instructions, instr_lengths,
        ...
        if (not self.params["train_action_only"] or not self.params["train_action_from_dstar"] or not self.params["run_action_from_dstar"])\
            ...
            # print()
            # print("features_w: ", features_w, "\n", "coverages_w: ", coverages_w)
