...
from learning.modules.sentence_embeddings.sentence_bert_cosine_similarity import SentenceBert
from learning.modules.sentence_embeddings.navi_clip import NaviCLIP
from learning.modules.vit.vision_transformer import Vit4Similarity
...
class ModelTrajectoryTopDown(ModuleWithAuxiliaries):
    ...
    def __init__(self, run_name="", model_class=MODEL_FPV):
        ...
        # Vision Encoder
        self.vision_encoder = Vit4Similarity()
    ...
    def forward(self,
        ...
        if instructions is not None:
            if instr_len is not None:
                ...
                sent_emb, sent_emb_with_step = self.sentence_embedding(instructions)
            self.keep_inputs("sentence_embed", sent_emb)
            self.keep_inputs("sent_emb_with_step", sent_emb_with_step)
        else:
            sent_emb = self.sentence_embedding.get()

        # Calculate the image encoding
        img_encoding = self.vision_encoder(raw_images)
        self.keep_inputs("image_embed", img_encoding)
    ...
    def sup_loss_on_batch(self, batch, eval):
        ...
        if self.model_class in [MODEL_FPV, PVN_STAGE1_ONLY, MODEL_FPV_SAVE_MAPS_ONLY]:
            ...
            raw_images = self.maybe_cuda(batch["raw_imgs"])
        else:
        ...
        for b in range(batch_size):
            ...
            b_raw_images = raw_images[b][:b_seq_len]
        ...
        if self.params["run_auxiliaries"]:
            ...
            # calculate clip loss
            navi_clip_loss = self.cal_cosine_loss("sent_emb_with_step", "image_embed")
            aux_loss += navi_clip_loss
