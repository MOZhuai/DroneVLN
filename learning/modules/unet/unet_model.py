...
class DoubleConv(torch.nn.Module):
    def __init__(self, cin, cout, k, stride=1, padding=0, half=False):
        super(DoubleConv, self).__init__()
        if half:
            self.conv1 = nn.Conv2d(cin, int(cin / 2), k, stride=stride, padding=padding)
            self.conv2 = nn.Conv2d(int(cin / 2), cout, k, stride=1, padding=padding)
        else:
...
class UnetModel(torch.nn.Module):
    def __init__(self, in_channels, out_channels, embedding_size, hc1=32, hb1=16, hc2=256, stride=2):
        ...
        self.conv1 = DoubleConv(in_channels, hc1, 3, stride=stride, padding=1, half=True)
...
if __name__ == '__main__':
    ...
    emb = torch.zeros((3, 284))
    x_in = torch.zeros((3, 1024, 32, 32))
    out = model(x_in, emb)
