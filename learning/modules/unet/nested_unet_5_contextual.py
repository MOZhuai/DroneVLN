...
class NestedUnet5ContextualBneck(torch.nn.Module):
    def __init__(self, in_channels, out_channels, embedding_size, hc1=32, hb1=16, hc2=256, stride=2, split_embedding=False):
        ...
        # down-sampling
        self.conv0_0 = DoubleConv(in_channels, hc1, 3, stride=stride, padding=1)
        self.conv1_0 = DoubleConv(hc1, hc1, 3, stride=stride, padding=1)
        self.conv2_0 = DoubleConv(hc1, hc1, 3, stride=stride, padding=1)
        self.conv3_0 = DoubleConv(hc1, hc1, 3, stride=stride, padding=1)
        self.conv4_0 = DoubleConv(hc1, hc1, 3, stride=stride, padding=1)

        self.conv0_1 = DoubleConv(hc1*2, hc1, 3, stride=stride, padding=1)
        self.conv1_1 = DoubleConv(hc1*2, hc1, 3, stride=stride, padding=1)
        self.conv2_1 = DoubleConv(hc1*2, hc1, 3, stride=stride, padding=1)
        self.conv3_1 = DoubleConv(hc1*2, hc1, 3, stride=stride, padding=1)

        self.conv0_2 = DoubleConv(hc1*3, hc1, 3, stride=stride, padding=1)
        self.conv1_2 = DoubleConv(hc1*3, hc1, 3, stride=stride, padding=1)
        self.conv2_2 = DoubleConv(hc1*3, hc1, 3, stride=stride, padding=1)

        self.conv0_3 = DoubleConv(hc1*4, hc1, 3, stride=stride, padding=1)
        self.conv1_3 = DoubleConv(hc1*4, hc1, 3, stride=stride, padding=1)

        self.conv0_4 = DoubleConv(hc1*5, hc1, 3, stride=stride, padding=1)

        # up-sampling
        self.deconv1_0 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv2_0 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv3_0 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv4_0 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)

        self.deconv1_1 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv2_1 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv3_1 = DoubleDeconv(hc1+hb1, hc1, 3, stride=stride, padding=1)

        self.deconv1_2 = DoubleDeconv(hc1, hc1, 3, stride=stride, padding=1)
        self.deconv2_2 = DoubleDeconv(hc1+hb1, hc1, 3, stride=stride, padding=1)

        # self.deconv1_3 = DoubleDeconv(hc1+hb1, hc1, 3, stride=stride, padding=1)
        self.deconv1_3 = DoubleDeconv(hc1+hb1, hc2, 3, stride=stride, padding=1)

        # self.deconv0_4 = nn.ConvTranspose2d(hc1+hb1, out_channels, 3, stride=stride, padding=1)
        self.deconv0_4 = nn.ConvTranspose2d(hc2+hb1, out_channels, 3, stride=stride, padding=1)
        ...
        self.norm0_0 = nn.InstanceNorm2d(hc1)
        self.norm1_0 = nn.InstanceNorm2d(hc1)
        self.norm2_0 = nn.InstanceNorm2d(hc1)
        self.norm3_0 = nn.InstanceNorm2d(hc1)
        self.norm4_0 = nn.InstanceNorm2d(hc1)

        # norm in decoder process
        self.dnorm0_1 = nn.InstanceNorm2d(hc1)

        self.dnorm1_1 = nn.InstanceNorm2d(hc1)
        self.dnorm0_2 = nn.InstanceNorm2d(hc1)

        self.dnorm2_1 = nn.InstanceNorm2d(hc1)
        self.dnorm1_2 = nn.InstanceNorm2d(hc1)
        self.dnorm0_3 = nn.InstanceNorm2d(hc1)

        self.dnorm3_1 = nn.InstanceNorm2d(hc1)
        self.dnorm2_2 = nn.InstanceNorm2d(hc1)
        self.dnorm1_3 = nn.InstanceNorm2d(hc1)
        self.dnorm0_4 = nn.InstanceNorm2d(hc2)
        ...
        # self.fnorm5 = nn.InstanceNorm2d(hb1)
    ...
    def forward(self, input, embedding):
        ...
        # pool_x = self.pool(x0_0)
        # x1_0 = self.conv1_0(pool_x)
        # up_x = self.up(x1_0)
        # x0_1 = self.conv0_1(torch.cat([x0_0, up_x], 1))
